...
import type { CableSpec } from '@/cables'
...
export const useMuseumStore = defineStore('museum', {
  state: () => ({
    eras: [] as Era[],
    cableSpecs: {} as Record<string, CableSpec>,
  ...
  actions: {
    async loadContent() {
      ...
      const [eras, specs] = await Promise.all([
        fetch(`${base}content/cable_eras_v2.json`).then(r => r.json()),
        // Cable construction specs drive the cross-section + cut-away
        // diagrams. Generated by scripts/generate_cable_art.py.
        fetch(`${base}content/cable_specs.json`).then(r => r.json()).catch(() => ({}))
      ])
      this.eras = eras
      this.cableSpecs = specs
    },
    openEra(id: string) {
